...
def parse_manifest(manifest, base_path):
    audio_files = []
    codes = []
    pitch = []
    ...
    return audio_files, codes, pitch


def get_dataset_filelist(h):
    training_files, training_codes, training_pitch = parse_manifest(h.input_training_file, h.train_base_path)
    validation_files, validation_codes, validation_pitch = parse_manifest(h.input_validation_file, h.val_base_path)

    return (training_files, training_codes, training_pitch), (validation_files, validation_codes, validation_pitch)
...
class CodeDataset(torch.utils.data.Dataset):
    def __init__(self, training_files, segment_size, code_hop_size, n_fft, num_mels,
                 ...
                 f0_interp=False, vqvae=False, id_to_spkr=None, eval_mode=False):
        self.audio_files, self.codes, self.pitch = training_files
        random.seed(1234)
        ...
        if f0_stats:
            with open(f0_stats, 'rb') as f:
                self.f0_stats = pickle.load(f)  # torch.load(f0_stats)
        self.multispkr = multispkr
        self.pad = pad
        if self.multispkr:
            if id_to_spkr:
                self.id_to_spkr = id_to_spkr
            else:
    ...
    def __getitem__(self, index):
        ...
        if not self.eval_mode:
            ...
            pitch = self.pitch[index][:code_length]
        ...
        if self.f0_normalize:
            spkr_name = parse_speaker(self.audio_files[index], self.multispkr)
            if spkr_name not in self.f0_stats:
                mean = self.f0_stats['f0_mean']
                std = self.f0_stats['f0_std']
            else:
                mean = self.f0_stats[spkr_name]['mean']
                std = self.f0_stats[spkr_name]['std']
            ii = feats['f0'] != 0
